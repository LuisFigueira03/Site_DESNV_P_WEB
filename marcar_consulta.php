...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($data_consulta < date('Y-m-d')) {
        $erro = "Não é possível marcar consultas em datas passadas.";
    }
    else {
...
// Buscar especialidades, médicos e acordos
...
// Horário semanal e ausências
$horariosMedicos = [];
$sql = "SELECT medico_id, dia_semana, turno FROM horario_medico";
$stmt = $dbh->query($sql);
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $horariosMedicos[$row['medico_id']][$row['dia_semana']] = $row['turno'];
}

$ausenciasMedicos = [];
$sql = "SELECT medico_id, data_inicio, data_fim FROM ausencia_medico WHERE tipo='ferias' AND ativo=1";
$stmt = $dbh->query($sql);
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $ausenciasMedicos[$row['medico_id']][] = ['inicio'=>$row['data_inicio'],'fim'=>$row['data_fim']];
}
?>
<!DOCTYPE html>
<html lang="pt-PT">
...
<body class="bg-gray-100 text-slate-800 antialiased overflow-y-scroll">
...
<div class="min-h-screen flex flex-col">
  <main class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1"> 
    ...
    <header class="mb-6 flex items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl md:text-3xl font-extrabold tracking-tight text-[#09A2AE]">Marcação de Consulta</h1>
    ...
    <form method="post" action="" class="bg-white rounded-xl shadow p-6 space-y-6">
      <section>
        <h2 class="text-lg font-semibold mb-4 text-[#09A2AE]">Dados da consulta</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select id="especialidade" name="especialidade_id" required
              ...
              <?php foreach($especialidades as $esp): ?>
                <option value="<?= $esp['id'] ?>" <?= ($especialidade_id == $esp['id']) ? 'selected' : '' ?>>
          ...
          <div>
            ...
            <select id="profissional" name="medico_id" required disabled
                    class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#09A2AE]">
              <option value="">Selecione...</option>
          ...
          <div>
            ...
            <input id="data_consulta" name="data_consulta" type="date" required
          ...
          <div>
            ...
            <input type="hidden" id="hora_inicio" name="hora_inicio" value="<?= htmlspecialchars($hora_inicio) ?>">
            <div id="lista_horas" class="grid grid-cols-3 gap-2 text-sm min-h-[120px]"></div>
      ...
      <div class="flex justify-end">
        <button type="submit"
                class="rounded-md px-6 py-3 bg-[#09A2AE] text-white font-semibold hover:opacity-90">
          Confirmar marcação
        </button>
      </div>
    </form>
  ...
  <footer class="bg-[#09A2AE] text-white text-center">
...
<script>
  ...
  const horariosMedicos = <?php echo json_encode($horariosMedicos, JSON_UNESCAPED_UNICODE); ?>;
  const ausenciasMedicos = <?php echo json_encode($ausenciasMedicos, JSON_UNESCAPED_UNICODE); ?>;

  const TURNOS_HORARIOS = { 'M': ['08:00','09:00','10:00','11:00','12:00'], 'T': ['14:00','15:00','16:00','17:00','18:00'], 'N': ['19:00','20:00','21:00','22:00','23:00'] };
  ...
  function actualizarHoras() {
    ...
    const diaSemana = ['domingo','segunda','terça','quarta','quinta','sexta','sábado'][new Date(data).getDay()];
    const diaIndex = {'segunda':'seg','terça':'ter','quarta':'qua','quinta':'qui','sexta':'sex','sábado':'sab','domingo':'dom'}[diaSemana];

    const turnoDia = horariosMedicos[medicoId] ? horariosMedicos[medicoId][diaIndex] : 'F';
    if (turnoDia === 'F' || !turnoDia) {
        notaHoras.textContent = 'O médico não trabalha neste dia.';
        return;
    }

    let horasPossiveisDia = TURNOS_HORARIOS[turnoDia] || [];

    const horasOcupadas = (bookings[medicoId] && bookings[medicoId][data]) ? bookings[medicoId][data] : [];

    const ausencias = ausenciasMedicos[medicoId] || [];
    const estaAusente = ausencias.some(a => data >= a.inicio && data <= a.fim);
    if (estaAusente) {
        notaHoras.textContent = 'O médico encontra-se ausente neste dia.';
        return;
    }

    notaHoras.textContent = 'Clique numa hora para seleccionar.';

    horasPossiveisDia.forEach(horaStr => {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.textContent = horaStr;
      btn.className = 'slot w-full h-10 px-2 rounded-md border hover:opacity-90 text-center';
      ...
      btn.addEventListener('click', function () {
        ...
        listaHoras.querySelectorAll('.slot').forEach(b => b.classList.remove('bg-[#09A2AE]','text-white','border-[#09A2AE]'));
  ...
  function aplicarFiltroMedicos() {
    const espEscolhida = campoEspecialidade.value;
    campoMedico.disabled = !espEscolhida;

    Array.from(campoMedico.querySelectorAll('option')).forEach(opt => {
      if (opt.value === '') { opt.hidden = false; return; }
      const espMed = opt.getAttribute('data-esp-id');
      opt.hidden = espEscolhida ? (espMed !== espEscolhida) : true;
    });

    if (campoMedico.selectedOptions[0] && campoMedico.selectedOptions[0].value !== '') {
      const espSel = campoMedico.selectedOptions[0].getAttribute('data-esp-id');
      if (espEscolhida && espSel !== espEscolhida) campoMedico.value = '';
    }

    actualizarHoras();
  }

  campoEspecialidade.addEventListener('change', aplicarFiltroMedicos);
  ...
  window.addEventListener('load', function () {
    ...
    if (campoData.value && campoMedico.value) actualizarHoras();
  });
</script>
