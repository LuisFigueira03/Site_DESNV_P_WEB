        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p id="nota_horas" class="mt-1 text-xs text-slate-500">
              Escolha a data e a especialidade para ver profissionais e horários disponíveis.
            </p>
          </div>
        </div>
...
<script>
  ...
  const HORAS_POSSIVEIS = ['09:00','10:00','11:00','14:00','15:00','16:00','17:00'];
  ...
  function actualizarHoras() {
    ...
    HORAS_POSSIVEIS.forEach(function (horaStr) {
      ...
      btn.className = 'slot w-full h-10 px-2 rounded-md border hover:bg-slate-50 text-center';
  ...
  function aplicarFiltroMedicos() {
    ...
    const opcoes = Array.from(campoMedico.querySelectorAll('option'));
    opcoes.forEach(opt => {
    ...
    // Se o médico seleccionado não pertence à especialidade, limpa
    const optSel = campoMedico.selectedOptions[0];
    if (optSel && optSel.value !== '') {
      const espDoSel = optSel.getAttribute('data-esp-id');
      if (espEscolhida && espDoSel !== espEscolhida) {
        campoMedico.value = '';
      }
    }
  ...
  window.addEventListener('load', function () {
    ...
    // Se já vier médico preenchido do POST, e a especialidade estiver escolhida, mostra horas
